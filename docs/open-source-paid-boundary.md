...
- Core wire DTOs/schemas are owned in `src/contracts.ts` and carry explicit v1 identifiers; published API responses use those DTOs rather than raw PocketBase records.
...
- Signed data uses `SignedTestSnapshot` with a declared canonicalization version and SHA-256 `recordHash`. Lookahead verifies/retains the hash but never redefines the canonical bytes.
- Core emits or exposes versioned facts such as `test.locked`, `test.voided`, and `export.generated`. Notification routing/delivery belongs outside core.
...
Open-source contract tests and fictional payload fixtures are the handoff artifact. Do not copy TypeScript source across repositories and allow it to diverge; publish or consume a tagged contract artifact when an integration actually exists.
