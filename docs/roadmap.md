...
| R2-12 Report evidence thumbnails | Reviewers want to see gauge/calibration photos without opening each attachment. | Image-type evidence renders an inline thumbnail (fetched from protected storage) alongside its reference line while documents stay reference-only; output stays deterministic in field content, the snapshot hash remains the integrity anchor, and no incidental PII (e.g. capture GPS) is embedded. Extends the reference-only v1 report ([`report-field-spec.md`](report-field-spec.md)). | R1-10 | Core | Post-MVP |
